document.addEventListener('DOMContentLoaded', () => {
	...
	// ========== BUSCAS DE API COM FALLBACK (CONTINGÊNCIA) ==========
    ...
    async function buscarCNPJ() {
        ...
        // TENTATIVA 1: Open CNPJ (Sua API original)
        try {
            console.log("Tentando buscar na Open CNPJA...");
            const response = await fetch(`https://open.cnpja.com/office/${documento}`);
            
            if (!response.ok) throw new Error("Falha na API 1");
            ...
            if (data.company) {
                ...
                showToast('Dados carregados (Base 1)', 'success');
            ...
            console.warn("API 1 falhou, tentando BrasilAPI...");
            
            try {
                const response = await fetch(`https://brasilapi.com.br/api/cnpj/v1/${documento}`);
                
                if (!response.ok) throw new Error("Não encontrado em nenhuma base");
                ...
                showToast('CNPJ não encontrado em nenhuma base de dados', 'error');
            }
        } finally {
            hideLoading('btn-cnpj-d');
        }
    ...
    // Função auxiliar para preencher os campos (evita repetir código)
